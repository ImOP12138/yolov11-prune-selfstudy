...
def main(opt):
    ...
    参考YOLOv8剪枝脚本的逻辑, 不再受个别BN层gamma值的限制
    """
    print(colorstr('blue', 'Calculating pruning threshold...'))
    # 计算按照当前用户指定比率剪枝下的阈值
    # 对sorted_bn进行降序排序，以便与YOLOv8的逻辑一致
    sorted_bn_desc = torch.sort(torch.tensor(bn_weights), descending=True)[0]
    thre = sorted_bn_desc[int(len(sorted_bn_desc) * prune_ratio)] if prune_ratio >= 0 else -1
    print(f'Pruning threshold: {colorstr(f"{thre:.4f}")}')
    print(f'Pruning ratio: {colorstr(f"{prune_ratio:.3f}")}')
    ...
    for name, module in model.model.named_modules():
        if isinstance(module, nn.BatchNorm2d):
            ...
            if name not in ignore_bn_list:
                # 对每个BN层单独计算阈值，参考YOLOv8的逻辑
                gamma = module.weight.data.abs()
                # 按权重排序，取前(1-prune_ratio)的通道
                sorted_weights, indices = torch.sort(gamma, descending=True)
                # 计算需要保留的通道数
                target = max(int(len(gamma) * (1 - prune_ratio)), 8)
                # 确保至少保留8个通道
                if target < 8:
                    target = 8
                # 确保通道数是8的倍数
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default='E:\\tools\\jupyter_project\\op_projects\\YOLOdemo\\visdrone.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train-sparsity-1080/weights/last.pt', help='model.pt path(s)')
    parser.add_argument('--cfg', type=str, default=ROOT / 'ultralytics/cfg/models/11/yolo11.yaml', help='model.yaml path')
    parser.add_argument('--model-size', type=str, default='n', help='(yolov11)n, s, m, l or x?')
    parser.add_argument('--prune-ratio', type=float, default=0.8, help='prune ratio, leave it -1 if you dont want to prune(for example, debug purpose)')
    parser.add_argument('--save-dir', type=str, default=ROOT / 'weights-pruned-1080', help='pruned model weight save dir')
